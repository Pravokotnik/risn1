...
class FaceFilter(Node):
    def __init__(self):
        ...
        self.position_history = defaultdict(lambda: {
            'points': deque(maxlen=self.min_cluster_size+5),  # Stores tuples of (position, destination, robot_pose_at_detection, gender)
            'position': None,            # Current average position
            'destination': None,         # Current average destination
            'gender': None,              # Most common gender in cluster
        ...
        self.face_queue = deque()               # will hold (FaceMsg, frame_id, stamp)
    ...
    def marker_callback(self, msg):
        ...
        if distance < 0.15:
            ...
            return
        self.get_logger().info(f"Received face detection at {target_point.x}, {target_point.y}, {target_point.z} with distance {distance:.2f}m, gender: {msg.gender}")
    ...
    def _process_face_queue(self):
        ...
        for i, (msg, frame_id, stamp) in enumerate(self.face_queue):
            ...
            gender = msg.gender  # Extract gender from message
    ...
    def _handle_mapped_face(self, map_point, bottom_left_point, bottom_right_point, top_left_point, top_right_point, gender):
        # Transform face point into map frame
        if not map_point:
            return
        
        self.get_logger().info(f"Detected face at {map_point.x}, {map_point.y}, {map_point.z}, gender: {gender}")
        ...
        self.process_clusters(map_point, destination, gender)  # Pass gender to clustering
    ...
    def get_most_common_gender(self, points):
        """Get the most common gender from a list of points"""
        gender_counts = {}
        for point in points:
            if len(point) > 3:  # Check if gender is available
                gender = point[3]
        ...
        return max(gender_counts, key=gender_counts.get)

    def process_clusters(self, position, destination, gender):
        """
        ...
        for cluster_id, cluster_data in self.position_history.items():
            ...
            if pos_distance < self.cluster_radius and dest_distance < self.dest_radius:
                ...
                for i, point_data in enumerate(cluster_data['points']):
                    point, _, detection_pose = point_data[:3]  # Handle both old and new format
        ...
        if matched_cluster is None:
            ...
            if self.is_new_emergency(position):
                ...
                self.position_history[matched_cluster]['gender'] = gender
        ...
        self.position_history[matched_cluster]['destination'] = self.average_destination(
            self.position_history[matched_cluster]['points'])
        self.position_history[matched_cluster]['gender'] = self.get_most_common_gender(
            self.position_history[matched_cluster]['points'])
    ...
    def publish_task(self, cluster_id):
        ...
        # Include gender in task description
        gender = cluster.get('gender', 'unknown')
        task.description = f"Face cluster {cluster_id} - Gender: {gender}"
